...
| WS-23 | **Skills registry + per-agent skill toggles** (added 2026-08-01) | `specs/skills_registry.md` | 🟢 S1/S2 | Skill families as code-declared data + token-cost catalog (Integrations → Skills tab) + per-agent toggles (Agent Registry panel); **intersection-only** — narrows within declared scope, core floor non-toggleable; addendum generated from enabled set (kills B2 prose drift). **OWNER-GATE:** S3's fail-closed default flip. Per-instance profiles defer to Centers C; manifest side lands with WS-8. |
