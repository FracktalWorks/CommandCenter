...
| WS-4 | **Event-bus consumer + durable queue** (BO-20) | `FOUNDATION_BUILDOUT_CHECKLIST.md` §BO-20 — **the file is at the REPO ROOT, not under `ai-company-brain/`** (this row's old anchor was wrong) | 🟡 BO-20f built · 🔴 a–e | **State cell:** §2's legend has no single state for *"one sub-item built, the rest blocked on an owner call"*, so this cell composes the two that are individually true — **BO-20f is BUILT (2026-08-02, pending review)**: Gmail + Zoho receivers now `enqueue` to `ingestion:{gmail,zoho}` and schedule `emit_event` on `BackgroundTasks` in ClickUp's shape, pinned by `tests/unit/test_ingestion_receiver_parity.py` (22 tests) and `tests/unit/test_clickup_normalise_dlq.py` (4 tests) — no Redis/DB/network in either, closing the **code-side** dead switch where `TriggerPanel.tsx:266-269` had offered `gmail`/`zoho` as event-trigger sources that could never fire. ⚠️ **Code-side only — the feature is INERT in every environment today.** `zoho_webhook_secret` and `gmail_pubsub_token` both default to `""` and both receivers fail closed on unset, so every push 401s and nothing is enqueued or emitted; neither key exists in `.env.example` or on the VPS, and no Zoho webhook / Pub/Sub subscription points at the routes. **OWNER-GATE (an agent can do neither):** provision `ZOHO_WEBHOOK_SECRET` + `GMAIL_PUBSUB_TOKEN` on the VPS (`.env.example` itself is OWNER-GATE under WS-2 — the plan-guard hook blocks agent writes to it) **and** point the provider subscription/webhook at `/webhooks/{zoho,gmail}`. Until both happen a user who binds a workflow to `source=gmail` sees nothing, with the only signal in Google's/Zoho's delivery dashboard. The fail-closed posture is correct and must not be changed. That PR also fixed a latent **500 on every authenticated Zoho push** (structlog `event=` kwarg collision) that the first `TestClient` drive-through exposed, and the last remaining instance of that bug class (`clickup/webhook.py`, where it dead-lettered every successfully-normalised task). §BO-20 moved ☐ → ◑. BO-20a–e are still 🔴 blocked on §BO-20.0. **Audited 2026-08-02 → NO-GO; §BO-20 rewritten the same day and now carries acceptance criteria, verification commands, gate labels and anchors.** ⚠️ **NOT a greenfield build: webhook→run is ALREADY wired.** ClickUp → `ingestion/event_hooks.emit_event` → `workflows/triggers.dispatch_event` → `start_run` has been live since commit `e20ea830`, and `/agent/webhook/{source}` is a second live path — the old row's "trigger no agent" premise died there, and an implementer handed it would have built a parallel dispatch path. What IS missing is narrower: `xreadgroup`/`xack` exist **nowhere in the repo**, so `ingestion:*` and `ingestion:dlq` are write-only and trimmed unread (no durability, no replay, no retry, no drainable DLQ, no rate limit, no bounded concurrency), and (until BO-20f shipped on 2026-08-02) only 1 of 3 receivers emitted at all. **🔴 ONE OWNER DECISION BLOCKS BO-20a–e (not f) — §BO-20.0, the process model:** *Option A, an in-process supervised asyncio consumer started from the gateway lifespan* (**recommended** — `workflows_app.md` §9 D6 names this the house style and says "Revisit under BO-20"; five such loops are already wired into the lifespan; ships by ordinary merge) vs *Option B, a separate `python -m ingestion.worker` process* (needs a new systemd unit on the VPS — **an agent cannot deploy it**, so that PR would merge and do nothing; picking B re-labels the deployment half of BO-20a–e OWNER-GATE). Answer with one line: `BO-20 = Option A` or `BO-20 = Option B` — but note that **only Option A unblocks BO-20a–e as written**; Option B leaves the dispatch step undefined (a separate process starts with an empty `event_hooks._SINKS`, so it would `XREADGROUP`, `XACK` and dispatch to nothing), so answering B buys a design round plus a rewrite of BO-20a, not a dispatch. **BO-20f (Gmail/Zoho receivers reach ClickUp's enqueue+emit parity) is the sub-item "multi-channel event triggers" actually need** — it needed no consumer, no flag and no owner decision, and **shipped 2026-08-02**. ⚠️ **It does NOT unblock WS-11 Slice 4**, which `workflows_app.md:217` defines as "(post-BO-20/BO-7): durable queued runs; sandboxed module execution; MCP exposure; retention policies" — durable queued runs need the consumer + retry + DLQ, so **Slice 4 needs BO-20a–e (plus BO-7) and still waits on §BO-20.0**; WS-11's "Slice 4 after WS-4" stays blocked exactly as before. BO-9 resolved as **not blocking** (a consumer needs a long-lived async client; the precedent is `acb_common/activity.py:54-66`). **OWNER-GATE:** flipping `INGESTION_CONSUMER=1` (the code ships OFF; **registered in §6 on 2026-08-02** — the flip also cuts the receivers over to enqueue-only, per §BO-20 Q1). |
