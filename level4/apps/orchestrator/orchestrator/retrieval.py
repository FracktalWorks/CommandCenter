    ...
    *,
    ...
    for tok in tokens:
        ...
        for person in repo.find_people_by_text(session, tok, limit=per_kind):
            ...
            # Person was named in the query → surface their tasks too, so
            # questions like "show me all tasks under Suryansh" actually have
            # tasks in context (otherwise the LLM only sees the bare Person
            # line and correctly says it doesn't know).
            for t in repo.tasks_for_owner(session, person.id, limit=per_kind):
                hits.append(_hit("task", t.id, _task_line(t)))
