...
emit_generative_ui(ui) — render rich UI inline. 3 modes: (1) component tree card/table/keyValue/badge/callout/buttons + an icon node (type:icon, name=any Lucide icon e.g. 'cloud-sun'); (2) a template node (type:template, name= weatherCard/statDashboard/barChart/sparkTrend/comparison/progressTracker) pre-designed animated cards, supply data only (stats take optional icon); (3) an html node (type:html, props code + optional icons list of Lucide names) custom animated HTML/CSS/JS in a sandbox — use declared icons via ccIcon('Name') or a span with data-cc-icon='Name'. Prefer template over tree over html.
...
- **emit_generative_ui(ui)** — Render a rich, interactive, animated UI element inline in the chat, on the fly. Use whenever a visual answer beats prose. `ui` is a JSON object discriminated by its top-level `type`. Three modes, prefer them in this order:
    1. **Named template** (best-looking, use first when one fits) — a node with type "template" and props holding `name` + a `data` object. Pre-designed animated cards; supply data only. Available names: weatherCard, statDashboard, barChart, sparkTrend, comparison, progressTracker (see the emit_generative_ui tool doc for each one's data shape). statDashboard stats accept an optional `icon` (a Lucide name).
    2. **Component tree** (structured, safe) — a whitelist tree of card / table / keyValue / badge / callout / list / button / icon nodes; data, not code. The icon node takes a `name` = any Lucide icon (e.g. cloud-sun, check-circle, trending-up). Good for summaries, tables, labelled rows, and action buttons.
    3. **Custom HTML** (escape hatch, only when no template/tree fits) — a node with type "html" and props holding `code` (a full HTML/CSS/JS snippet) plus an optional `icons` array of Lucide names. Runs in an isolated sandbox for bespoke animation/layout. No external network/CDNs — inline everything; use declared icons via ccIcon('Name') or a span with data-cc-icon='Name'. For interactivity, put data-cc-action='<follow-up message>' on a clickable element (or call ccAction('...') from your script) — it's sent as the user's next message, same as a tree button's action.
