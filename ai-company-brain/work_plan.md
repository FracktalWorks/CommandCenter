...
| WS-9 | **Memory tiers 3b/3c/4** (budgeted file-tier header, provenance markers, correction UX, supersession) | `memory_architecture.md` §9 (corrected 2026-08-01) | 🟢 | 3a′ substrate shipped (migs 136–139), spec statuses now marked. §6.7 correction UX is the highest-leverage UX item in the corpus. |
| WS-10 | **Multiplayer Phase 2 + 3b** (floor control, steer, `subject:` compartments, `prefs`/`user` backfill) | `docs/multiplayer/README.md` §8 + `memory-clearance.md` (stamped 2026-08-01) | 🟢 | README's 12 stale claims corrected; shipped surface + role vocabulary now stated in-doc. |
...
| WS-23 | **Skills registry + per-agent skill toggles** (added 2026-08-01) | `specs/skills_registry.md` | 🟡 S1 built | **S1 shipped pending review 2026-08-01**: `acb_skills/skill_families.py` registry + measured token-cost catalog, `GET /integrations/skills`, Integrations → Skills tab, drift test (`test_skills_registry.py`); family set corrected to code (core floor subsumes files/notes/web-research; email/whatsapp/tasks/calendar are repo-baked, not injected — see spec §4). Measured baseline: injected tool context ≈19.3k tokens (addendum ≈5.7k), core floor ≈15.1k. Next: S2 toggles (settings table + PUT + Agent Registry panel; **intersection-only**, core floor non-toggleable). **OWNER-GATE:** S3's fail-closed default flip. Per-instance profiles defer to Centers C; manifest side lands with WS-8. |
...
| WS-18 | **Tasks Phase 3** (Weekly Review, Waiting-For, Horizons) | `task_manager_app.md` (corrected 2026-08-01) | 🟢 | Status reconciled against repo: `/tasks/sync`, EngageView, AssistantRail confirmed shipped; WeeklyReview/WaitingForView/HorizonsView/ProjectPlanner confirmed absent. |
