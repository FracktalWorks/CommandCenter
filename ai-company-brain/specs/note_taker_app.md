...
- **Consistent live speakers:** the **live speaker registry** (`routes/notes/live_speakers.py`) runs each ingested segment through a per-meeting *running voiceprint gallery* (cosine match/enroll) to assign a **stable** speaker id across chunks, and binds names live from self-introductions ("I'm Priya") — so consumers see *who* is speaking in real time. `GET /notes/meetings/{id}/live/roster` returns the current roster. The batch re-pass on stop remains authoritative and reconciles any live-diarization drift.
