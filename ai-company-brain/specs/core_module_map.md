...
|---|---|---|---|---|---|
| A1 | **Agent loop / executor** — run_agent_stream/run_agent, 3-tier dispatch (native MAF stream / Copilot SDK / batch shim), watchdogs, retries | `apps/orchestrator/orchestrator/executor.py` (4.5k lines) | Agent Loop | **B−** | Monolith; 4 duplicated stream paths (HH-8); no loop detection; failed tools persist as `done` (review P0-5) |
| A2 | **Event translation** — AG-UI event mapping, `_native_fc_events`, `_FcStreamState`, per-message dedup | `executor.py` + `chat/route.ts` translator | Agent Loop / AG-UI | **C+** | Same logic in 4 places + client + server; "one event translator" refactor queued (HH-8) |
...
| # | Module | Lives in | List section | Grade | Key gaps |
|---|---|---|---|---|---|
| D1 | **Chat API layer** — SSE proxy/translator, session+message persistence, respond-input proxy, Mem0 extraction hook | `workbench/.../api/agent/chat/route.ts`, `api/chat/sessions/*` | Agent Loop (context assembly resp.) | **C+** | Route owns translation+persistence+history — the "one persistence owner" refactor target; no persistence on reconnect path (P0-3) |
