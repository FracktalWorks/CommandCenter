...
|---|---|---|---|---|
| 2.1 ◐ | Collapse the two sync cores: `trigger_sync` becomes a thin wrapper over `_sync_account`; label-change learning becomes a post-sync hook. **PARTIAL:** the webhook (`sync.py:484`) already delegates to `_sync_account`, but `trigger_sync` (`sync.py:163`) still hand-inlines a duplicate fetch→persist→reconcile→watermark (dup blocks `sync.py:247-387`); refresh-token loss was patched on the duplicate, not collapsed. Label-learning (`_learn_from_label_changes`, `sync.py:70`) runs ONLY from `trigger_sync` — the scheduler path (which actually runs every ~300s) never learns. **Remains:** delegate `trigger_sync`→`_sync_account` + delete dup; add a `learn_label_changes` slot to `PostSyncHooks` and capture the per-message `old_categories` diff in `_sync_account`'s persist so the hook can run. | Kills refresh-token loss on manual sync, cursor drift, and revives label-learning (currently dead in production — scheduler path never runs it) | M-L | review §3.2 |
| 2.2 ◐ | `classify_and_apply()` wrapper centralizing match → conversation-resolve → apply → watermark; callers pick cost policy; `approved_includes_only` an explicit engine parameter. **PARTIAL:** `classify_matches` (`engine.py:832`) unifies match→resolve and the run-message bypass is fixed; `approved_includes_only` done (`engine.py:523`). **Remains:** apply+watermark is still 3 hand-rolled copies (`runner.py` 1222/1227, 1765/1784, 1915/1966); `_process_past_emails_job` still bypasses `classify_matches` (calls raw match helpers `1742`/`1745`, handles #110 via separate oldest-first `_project_thread_status_for_backfill` — deliberate, but outside the wrapper). Fold the 3 apply+watermark blocks into the wrapper; route or formally carve out process-past. | The #110 invariant enforced in ONE place instead of 2-of-5 call sites; closes the run-message/process-past bypasses and the unreviewed-pattern blast radius | M | review §3.1, §2.2 |
| 2.3 ✗ | Split `runner.py` → `actions.py` / `learning.py` / `jobs.py` + HTTP; split `replyzero.py` → `chat.py` / `thread_status.py` (the authority) / `followups.py` + views. Mechanical, import-only; fix the `LIKE '%sender%'` evidence collision while moving `learning.py`. **NOT STARTED** (runner.py 2316 lines, replyzero.py 2167; `jobs.py` already extracted). **Sequencing note:** these are the exact two files 2.1/2.2 rewrite — do the split AS PART OF the Tier-A work, not before it, to avoid split-then-churn. | 4,000 lines of mixed concerns; kills every lazy per-row import; the chat SSE code (~390 lines) finally leaves Reply Zero | M | review §3.1 |
...
| 2.6 ◐ | `internet_message_id` column + `$select` + upsert dedupe (prefer newest provider id, carry categories/watermark) + one-off merge of ~110 ghost pairs. **Schema+code DONE** (mig 89 + ingest reclaim by internet_message_id). **Remains:** the one-off ghost-merge pass over legacy NULL-id rows (a `scripts/` pass like the #112 repair, run after ids backfill) — Tier A (mutates live rows). | Kills duplicate classification of Outlook-rekeyed messages and the ghost rows skewing thread heuristics | M | review §3.1 |
