...
5. **Route existing writes through the broker** — ◑ **PARTIAL (`b1a5070`)**: the ClickUp writes (`create_task`/`update_task`/`create_project`) now flow through `BaseTaskProvider._broker_gate` → `propose()` audits + chokepoints every write and AUTO-APPLIES it (they are already user-approved, so no behaviour change). An `ACTION_BROKER_ENFORCE` kill-switch (default off) can flip an action to the approval queue. **Still to do here:** a **persistent handler** so a queued write can actually execute on approval (the gate enqueues but can't yet re-run the write from the inbox); the email-send path; and the Next.js inbox pane. Original notes:
