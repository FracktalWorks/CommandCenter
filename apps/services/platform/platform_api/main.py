...
class CompletionRequest(BaseModel):
    ...
    Deliberately permissive about the rest of the body: the Router is a
    pass-through, and a provider parameter it does not recognise is not its
    business to reject. What it does own is `model`, which must name a tier.
    """

    model: str = "tier-balanced"
    messages: list[dict[str, Any]]
    #: Correlates this call with the usage row, and makes a retry idempotent.
    #: Generated by the caller because only the caller knows a retry IS one.
...
async def chat_completions(req: CompletionRequest, caller: KeyCaller) -> Any:
    ...
    That is the whole argument for a Router (§4.1).

    ``billed_credits`` is 0 and no balance is checked: CP-6 adds the rate card
    and the gate, against burn this slice will have measured. Shipping the
    pricing first would mean setting it from estimates.
    """
    org_id = caller.organization_id
    ...
    with get_engine().begin() as conn:
        try:
            ...
            # 400, not a silent coercion to a default tier. A misconfigured
            # agent must be visible rather than quietly billed (D32.7).
            raise HTTPException(
                status_code=400,
    ...
    passthrough = req.model_dump(exclude={"model", "request_id"}, exclude_none=True)
    call_kwargs: dict[str, Any] = {
        **passthrough, "model": resolved.model, "api_key": secret,
    }
    if api_base:
        call_kwargs["api_base"] = api_base

    response = await call_provider(**call_kwargs)
    ...
    # product problem is worse. CP-6's gate is where unmeterable traffic becomes
    # visible rather than silent.
    try:
        usage = usage_from_response(response)
        request_id = req.request_id or f"auto-{uuid.uuid4().hex}"
        with get_engine().begin() as conn:
            store.record_usage(
                conn, org_id=org_id, request_id=request_id,
    ...
    # Returned unchanged. This is the choke point every agent runtime already
    # POSTs through, so the body must stay byte-identical to what the provider
    # sent or the Router becomes a compatibility problem for every caller.
    return response
