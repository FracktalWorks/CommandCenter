...
> **Update 2026-08-02 (WS-18, Waiting-For surfacing):** §9.1 slice 5 **Waiting-For** is now 🔄 — the *surfacing* half shipped: `ITEM_SELECT` projects `gtd_waiting.expected_by` / `last_nudged_at` (mig-48 columns that five INSERT sites wrote and nothing read), `GtdItemModel` carries them, and the `/tasks` **Waiting For** view is a dedicated surface grouped by WHO, each row rendering who / what / since-when with overdue (past `expected_by`, §6 line 540) and stale (>5 days, the same rule as `GET /tasks/insights`) flags. Predicates live in pure `workbench/control_plane/src/app/tasks/lib/waiting.ts`. `POST /tasks/items/{id}/delegate` now defaults `expected_by` from the item's own due date when the caller supplies none (the in-app Delegate dialog never asks for one, and a NULL there makes the row permanently unflaggable — "I asked them for it by my deadline" is the honest record). The Waiting-For rows drop the flat list's per-row `TaskCard` context menu (Schedule / Change stage / Mark as Done / Eliminate) and its chips in exchange for the who/what/since-when grouping; the focus pane still has the full editor, and Sort is hidden on this view because the grouping re-derives its own order. **No migration** — the substrate has existed since mig 48. Still open in §6 and NOT built: follow-up nudge drafting/sending (owner-gated: real sends) and delegation write-back (blocked on the Action Broker, BO-1). Slices 4 (Weekly Review) and 6 (Plan / Horizons) are unchanged and remain under-specified — `gtd_reviews.summary` is untyped JSONB and `gtd_horizons` has no link column, so both need spec work before dispatch.
> **Open item carried out of WS-18 — `expected_by` is write-once, so a renegotiated deadline makes the badge lie.** Every one of the five sites sets `expected_by` at the moment the waiting-for record is opened, and nothing ever revises it: the only `UPDATE gtd_waiting` in the codebase (`sync.py`, `email_link.py`) sets `resolved = true`, and no route or UI can edit the column. So when a deadline moves, `gtd_items.due_at` moves and `expected_by` does not — a row can read Overdue after the date was pushed out, or read fine after it was pulled in. The delegate default above improves the *initial* value; it does not close this. Closing it needs a decision this spec does not record, so it is deliberately NOT implemented: either the due-date PATCH path also updates the open `gtd_waiting` row (which couples two facts §6 keeps distinct — my deadline vs the date I asked them for), or the Waiting-For UI grows an editable expected-by so renegotiation is an explicit act with its own provenance. Decide which before dispatching it.
...
| 5 — Waiting-For / Delegate | F6 | 🔄 surfacing done 2026-08-02 (WS-18) | `WaitingForView.tsx` (grouped by WHO; who/what/since-when per row; overdue + stale flags) + pure `lib/waiting.ts` + `ITEM_SELECT`/`GtdItemModel` now reading `gtd_waiting.expected_by`/`last_nudged_at`. **Open:** nudge drafting/sending (owner-gated) · delegation write-back (BO-1) · `expected_by` is write-once, so a renegotiated deadline makes the flag lie — needs a spec decision first (see the WS-18 header note) |
