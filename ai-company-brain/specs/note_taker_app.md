...
**Summary-first workspace (2026-07-23):** the meeting detail moved from a cramped two-column grid to a **tabbed workspace** (`meeting/[id]/page.tsx`) — an at-a-glance **meta strip** (date · duration · N speakers · language · model) over **Summary / Transcript / Actions(N) / Ask** tabs, so the generated notes are the hero instead of one card among many. Ask citations switch to the Transcript tab and scroll+cue the audio (provenance preserved across tabs). **Resume point: live transcription via Deepgram's streaming WebSocket (captions during recording — the next big build); share-to-chat (last slice-2 item); then optional notes→transcript provenance highlighting.** The `meeting`/`action_item` tables from `01_schema.sql` L91–111 are active.
