...
**the six WS-27 Projects gates** (`specs/project_management_app.md`), (a)–(d)
registered 2026-08-05, (e) added 2026-08-08, **(f) added 2026-08-12**:
**(f) flipping `projects_enforce_transitions`** — the Project Operations lifecycle
state machine (WS-27bg, spec §11.35). It ships **default OFF**, which leaves every
legacy status mover exactly as it was; the new work routes enforce it regardless.
Flipping it makes `core.apply_status_transition` refuse an illegal move for the
PATCH route and the **board's drag-and-drop** too — so a card dragged from Delivered
straight back to Paused starts answering 422 instead of silently producing a task
that was completed and then paused without ever being resumed. That is the intended
end state (brief §40: transitions must not be bypassable through the API), but it is
a visible behaviour change on a surface people use every day. **An agent must refuse
to flip it.** Before flipping, check what the current data would newly refuse:
`SELECT s.category, count(*) FROM pm_tasks t JOIN pm_task_statuses s ON s.id =
t.status_id GROUP BY 1;` and read `operations.TRANSITIONS`.
