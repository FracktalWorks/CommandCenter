...
> **Status:** 🔄 slices 0 + 1 (intelligence) built (2026-07-23). **Slice 0:** migration `95_note_taker.sql` (validated against a clean PG16+pgvector chain run), `packages/acb_stt` (Groq/OpenAI/Deepgram BYOK providers + registry, unit-tested), gateway `routes/notes/` (meeting CRUD, upload → background transcription pipeline, audio playback), Next proxy, `/notes` library + meeting-detail shell. **Slice 1 (this pass):** notes generation on `acb_llm` — a template→prompt compiler (`templates.py`), grounded structured-JSON summarization with map-reduce over long transcripts and draft `action_item` extraction (`summaries.py`), auto-chained after transcription; SSE per-meeting progress (`events.py` + dedicated Next stream proxy); and a two-pane transcript↔notes meeting detail with live progress and an action-items rail. **Resume point: the remaining slice-1 piece — the browser recorder (MediaRecorder chunked upload, pause/resume, offline buffer) so capture happens in-app, not just via file upload; then slice 2 loop-closure into `/tasks` (§6).** The `meeting`/`action_item` tables from `01_schema.sql` L91–111 are active.
...
| D3 | STT | Pluggable `acb_stt` (cloud BYOK **and** self-host), not a single hardwired engine | Mirrors `acb_llm` BYOK philosophy; VPS-no-GPU reality demands Tier A day-1; privacy story demands Tier B; engines keep leapfrogging (appendix §4) — the interface outlives any winner |
