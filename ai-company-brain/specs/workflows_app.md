...
> **Status:** 🔄 Slices 1+2 built — data model (migration 132) + gateway API + MAF compiler/engine + `/workflows` visual editor + Module Studio + **Workflow Copilot (F14)** + **keyword capability search (F15; semantic → BO‑22)** + **event triggers (F10)** + **approval node with pause/resume via the Action Broker inbox (F11)**. All five trigger kinds live: manual, api, webhook, schedule, event.
...
| F3 | **Node catalog from the platform** — Agents (from the live registry), Integration actions (from `acb_skills` integrations), Logic (condition/switch, transform, set-variable, wait), Modules (F7), HTTP request, Output/notify | Must | Catalog served by the gateway, not hard-coded in the UI (G3) |
...
- **Slice 2 (shipped):** event triggers via `/agent/webhook/{source}` + the ClickUp receiver's event-hook sink; approval node via `workflow_run_pauses` snapshots + the Action Broker approvals inbox, with cached-replay resume. Remaining from the original slice: publish-gate write-class checks (runtime broker gating covers writes today) and streaming from the raw MAF event stream (engine-level per-node events stream today).
...
5. No workflow can be published with an unresolved `{{ref}}`, a secret-shaped string in node config, or (once the check ships) a write-class node without an approval ancestor.
