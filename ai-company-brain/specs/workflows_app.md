...
- **Slice 2:** event triggers bound to ingestion normalizers; approval node via `workflow_run_pause` + approvals inbox; publish-gate write-class checks; run streaming onto the canvas from real MAF event stream (v1 streams engine-level per-node events).
