...
✅ **Landed and verified on `claude/paca-research-task-management-a1f6zd` (`47207818`), safe to build on**: WS-27ak slice 1 (Modal, PR #429, not yet merged/deployed — see prior entry) · **WS-27bc's `pagedPicker.ts`** (pure module, 31 cases, 9/9 mutants killed, wired nowhere) · D-PM-24 (the `inert` gap accepted) · written acceptance criteria for WS-27ak items (2) Tooltip and (5) Skeleton.
🔴 **THREE AGENTS DISPATCHED IN PARALLEL, ALL THREE DIED ON THE SAME API-LIMIT ERROR, NONE FINISHED, NONE WERE VERIFIED. Two left real work; recovered as WIP branches — neither is merged, neither is trustworthy without a fresh audit.**
- **`ws-27ak-toast`** (pushed, commit `f79a0c9e`, diff vs `583a0d38`: 9 files, +1519/−21) — Toast primitive (`src/lib/toast.ts` + `.test.ts`, `src/components/ui/Toast.tsx`, `e2e/toast.spec.ts`), plus edits to `layout.tsx`, `NotificationBell.tsx`, `TableView.tsx`, `TaskPanel.tsx`, `conformance.test.ts`. Died mid-repair, at *"Now extending conformance rule 8"* — **the conformance/rule-count edit may be incomplete or half-applied. Re-run `npx vitest run src/lib/theme/` before trusting anything.** Nothing in this branch has been typechecked or run by anyone since the agent wrote it.
- **`ws-27be-trgm-search-index`** (pushed, commit `92748382`, diff vs `583a0d38`: 10 files, +951/−17) — a **new migration** `infra/postgres/170_projects_search_trgm.sql`, edits to `gateway/routes/projects/{filters,search}.py`, a live test `tests/live/live_ws27be.py`, three pytest files, spec + `infra/AGENTS.md` + `tests/live/README.md`. Died mid-verification, at *"Now final verification runs."* 🔴 **R1: the migration number `170` was NOT re-checked at merge — three collisions have happened in two weeks from exactly this. Re-verify `ls infra/postgres/` for the true next number before this migration goes anywhere near a database.** 🔴 **R8: nobody has run `EXPLAIN` against a real Postgres on this branch.** The whole point of this ticket was "hermetic fakes agree with whatever SQL they are handed" — an unverified migration touching the search path is the exact failure mode R8 exists to prevent. **Do not deploy this branch's migration without that check.**
- **`ws-27bf-theme-sweep`** — died before writing a single file (*"Now let me write a probe script to gather real data before designing assertions"*). Zero salvage; the ticket is untouched. **Still fully owed**: the four-theme sweep automation, the highest-leverage item in this queue, because it's the gate `CLAUDE.md` calls "the real gate" and six consecutive slices have owed it with none performed.
**On resume**: dispatch a `spec-auditor` (or read directly) against BOTH WIP branches before building on them — treat them as an unreviewed implementer's first draft, not as GO-verified work. Do not merge either into the working branch without independent `tsc`/`vitest`/live-DB verification first. Theme-sweep restarts from scratch, same brief as before (ruthless about false positives; record violations, don't fix them; land green via named exemptions).
➡️ **Next, in order**: (1) verify+merge or repair the two WIP branches, (2) build the theme sweep from scratch, (3) then whichever of Tooltip/Skeleton — acceptance criteria are now written for both, in spec §9.4.2. — and pair that decision with the DOM-test-substrate question, since every rendering done-when in Wave 2 is review-only without it. (2026-08-11) |
