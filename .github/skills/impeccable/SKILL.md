...
- **Fixed per-item min-widths on a must-stay-visible nav/toolbar.** A row of equal siblings that all must remain visible (bottom-nav tabs, a segmented toolbar) built with a hard `min-w-[Npx]` per item plus padding inside `justify-around` overflows the moment the summed minimum exceeds the smallest target viewport (320px). It's a latent regression: correct at 3 items, clipped when someone adds a 4th or 5th. Use `flex-1 min-w-0` so siblings distribute the row and can never overflow; reserve fixed min-widths + horizontal scroll for lists whose item count is genuinely unbounded.
