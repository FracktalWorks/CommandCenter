...
# Names that should become FOLDER moves on Outlook (inbox-zero parity) vs stay
# as category LABELs.
_MS_FOLDER_NAMES = {"Newsletter", "Marketing", "Receipt", "Notification", "Cold Email"}
_MS_ARCHIVE_NAMES = {"Marketing", "Cold Email"}


def test_actions_for_preset_outlook_files_cleanup_categories_into_folders() -> None:
    for p in m._PRESET_RULES:
        ...
        if p["name"] in _MS_FOLDER_NAMES:
            ...
            assert "LABEL" not in types, p["name"]
            assert ("ARCHIVE" in types) == (p["name"] in _MS_ARCHIVE_NAMES)
        else:
