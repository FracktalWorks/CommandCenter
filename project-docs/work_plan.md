...
passed. 🔴 **R1 FIRED AT MERGE — the migration was renumbered 172 → 175.** Merging `main` back in
brought WS-28's `172_people_profile.sql`, plus 173 and 174; both branches had taken 172 while in
flight, which is collision **number four in two weeks** and the exact case R1's re-check exists
for. ✅ **The renumber cost one `git mv` and a header line**, because
`test_projects_org_vocabularies.py` finds the migration **by content** (`"org-wide
vocabularies"`) rather than by number — the discipline R1 asks for, now demonstrated rather than
asserted. `tests/unit` re-run green after the renumber. (2026-08-14) |
