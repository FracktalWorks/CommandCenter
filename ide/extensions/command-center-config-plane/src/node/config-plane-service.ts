...
import {
    computeTrustScore,
    scanSkillContent,
    selectDirectivesForPrompt,
    SkillScanResult,
    TrustScore,
} from '../common/agent-intelligence';
import { INTEGRATION_KIND_SPECS } from './integration-specs';
 ...
 */
@injectable()
export class ConfigPlaneServiceImpl implements ConfigPlaneService {

    /** Keys whose values are treated as secrets and withheld from the client. */
    protected static readonly SECRET_PATTERN =
        /(SECRET|PASSWORD|TOKEN|API_KEY|MASTER_KEY|ENCRYPTION_KEY|CLIENT_SECRET|GITHUB_PAT|DATABASE_URL|ACCESS_TOKEN|REFRESH_TOKEN|PUBLIC_KEY|PRIVATE_KEY|AUTH_SECRET|AUTH_GOOGLE_ID)/;
    ...
    async getSnapshot(): Promise<ConfigPlaneSnapshot> {
        ...
        for (const def of ConfigPlaneServiceImpl.SECTIONS) {
            buckets.set(def.id, []);
        }
        ...
        for (const [key, value] of values) {
            ...
            const def = ConfigPlaneServiceImpl.SECTIONS.find(d => d.match(key));
            if (def) {
        ...
        const sections: ConfigSection[] = ConfigPlaneServiceImpl.SECTIONS.map(def => ({
            id: def.id,
            group: def.group,
            title: def.title,
    ...
    protected resolveEnvFile(): { file: string; usingExample: boolean } | undefined {
        let dir = process.cwd();
        for (let i = 0; i < 6; i++) {
    ...
    protected toEntry(key: string, value: string): ConfigEntry {
        const secret = ConfigPlaneServiceImpl.SECRET_PATTERN.test(key);
    ...
    protected emptySections(): ConfigSection[] {
        return ConfigPlaneServiceImpl.SECTIONS.map(def => ({
            id: def.id,
            group: def.group,
            title: def.title,
            description: def.description,
    ...
    protected store(): IntegrationStore {
        if (!this.storeInstance) {
            this.storeInstance = new IntegrationStore(this.resolveRootDir());
        }
    ...
    protected toolStore(): ToolStore {
        if (!this.toolStoreInstance) {
            this.toolStoreInstance = new ToolStore(this.resolveRootDir());
        }
    ...
    async getKindSpecs(): Promise<IntegrationKindSpec[]> {
        return INTEGRATION_KIND_SPECS;
    }
    ...
    async createIntegration(draft: IntegrationDraft): Promise<IntegrationRecord> {
        const record = await this.store().create(draft);
        this.syncTheiaSettings();
        return record;
    }

    async updateIntegration(id: string, patch: Partial<IntegrationDraft>): Promise<IntegrationRecord> {
        const record = await this.store().update(id, patch);
        this.syncTheiaSettings();
        return record;
    }

    async setIntegrationEnabled(id: string, enabled: boolean): Promise<IntegrationRecord> {
    ...
    async testIntegration(id: string): Promise<IntegrationTestResult> {
        ...
        try {
            switch (record.kind) {
                ...
                case 'webhook':
                    return this.describeUntestable(record,
                        'Webhooks are validated when an event is received or sent; there is no safe automatic probe.');
                case 'infra':
                    return this.describeUntestable(record,
                        'Infrastructure services need their native driver to connect. Verify the host/port and credentials manually.');
                default:
    ...
    protected async testApiIntegration(
        ...
        const authType = record.values.authType ?? 'none';
        const headers: Record<string, string> = { 'User-Agent': 'Command-Center/1.0' };
        if (authType === 'bearer' && secrets.apiKey) {
            ...
            const basic = Buffer.from(`${record.values.username}:${secrets.password ?? ''}`).toString('base64');
    ...
    protected async testMcpIntegration(
        record: IntegrationRecord,
        secrets: Record<string, string>
    ): Promise<IntegrationTestResult> {
        const transport = record.values.transport ?? 'stdio';
        if (transport !== 'http') {
            const cmd = [record.values.command, record.values.args].filter(Boolean).join(' ');
            return this.describeUntestable(record,
                `This is a stdio MCP server (${cmd || 'no command set'}). It is launched on demand and cannot be probed without starting it.`);
        }
    ...
    protected readonly pendingOAuthState = new Map<string, string>();

    async startOAuth(id: string): Promise<OAuthAuthorizationInfo> {
        const record = (await this.store().list()).find(r => r.id === id);
        if (!record) {
        ...
        if (!authorizationUrl) { missing.push('Authorization URL (authorizationUrl)'); }
        if (!clientId) { missing.push('Client ID (clientId)'); }
        if (!redirectUri) { missing.push('Redirect URI (redirectUri)'); }
        if (missing.length > 0) {
            const present = Object.keys(record.values).filter(k => k !== 'authType').join(', ') || '(none)';
            throw new Error(
                `Cannot start OAuth for '${record.name}': missing ${missing.join(', ')}. `
        ...
        const instructions =
            '1. Open the authorization URL in a browser and sign in / approve access.\n'
    ...
    async completeOAuth(id: string, code: string, state?: string): Promise<OAuthTokenResult> {
        const record = (await this.store().list()).find(r => r.id === id);
        if (!record) {
            return { ok: false, message: `No integration found with id '${id}'.` };
        }
        ...
        const body = new URLSearchParams({
            ...
            code: code.trim(),
            redirect_uri: record.values.redirectUri ?? '',
            client_id: record.values.clientId ?? '',
        });
     ...
     */
    async completeOAuthByState(code: string, state: string): Promise<{ result: OAuthTokenResult; integrationName?: string }> {
        if (!state) {
        ...
        return { result, integrationName: record?.name };
    }

    async refreshOAuthToken(id: string): Promise<OAuthTokenResult> {
        const record = (await this.store().list()).find(r => r.id === id);
        if (!record) {
            return { ok: false, message: `No integration found with id '${id}'.` };
        }
        const authType = record.values.authType ?? '';
        const secrets = await this.store().getDecryptedSecrets(id);
        ...
        if (authType === 'oauth2-client-credentials') {
            ...
            if (record.values.scope) { body.set('scope', record.values.scope); }
     ...
     */
    protected async exchangeToken(record: IntegrationRecord, body: URLSearchParams): Promise<OAuthTokenResult> {
        ...
        const secretsPatch: Record<string, string> = { accessToken };
        if (refreshToken) {
    ...
    async callIntegration(
        ...
        const MAX_BODY = 32_768; // characters — safe for most LLM context windows
        const record = (await this.store().list()).find(r => r.id === id);
        if (!record) {
        ...
        if (/^https?:\/\//i.test(path_)) {
            ...
            const base = (record.values.baseUrl ?? '').replace(/\/$/, '');
     ...
     */
    protected async ensureAccessToken(id: string): Promise<string> {
        const record = (await this.store().list()).find(r => r.id === id);
        if (!record) {
            throw new Error(`No integration found with id '${id}'.`);
        }
        ...
        const stillValid = !!secrets.accessToken && (Number.isNaN(expiresAt) || expiresAt - Date.now() > 60_000);
        if (stillValid) {
            ...
            for (const file of this.findSkillFiles(dir)) {
                try {
                    ...
                    if (summary && !byName.has(summary.name)) {
                        ...
                        summary.safety = scanSkillContent(this.stripFrontmatter(raw));
    ...
    async scanSkill(name: string): Promise<SkillScanResult> {
        const body = await this.getSkill(name);
        return scanSkillContent(body);
    }
    ...
    async writeSkill(draft: SkillDraft): Promise<SkillSummary> {
        const name = this.agentId(draft.name);
        if (!name) { throw new Error('Cannot derive a skill id from the given name.'); }
        if (!draft.description?.trim()) { throw new Error('A skill description is required.'); }
        if (!draft.body?.trim()) { throw new Error('A skill body is required.'); }
        ...
        const fm: string[] = ['---', `name: ${name}`, `description: "${esc(draft.description.trim())}"`];
        ...
        summary.safety = scanSkillContent(this.stripFrontmatter(content));
    ...
    protected findSkillFiles(dir: string, depth = 0): string[] {
    ...
    protected parseSkillSummary(raw: string, source: string): SkillSummary | undefined {
        ...
        return {
            ...
            whenToUse: fm.get('when_to_use')?.replace(/^['"]|['"]$/g, ''),
            domain,
    ...
    protected static readonly BUILTIN_AGENT_IDS = new Set(['assistant', 'agent-creator', 'reflector']);
    ...
    async listAgents(): Promise<AgentDefinition[]> {
        ...
        for (const file of files.filter(f => f.endsWith('.json'))) {
            try {
                ...
                agents.push(JSON.parse(raw) as AgentDefinition);
            } catch { /* skip malformed */ }
        }
        return agents.sort((a, b) => {
            if (a.id === 'agent-creator') { return -1; }
            if (b.id === 'agent-creator') { return 1; }
    ...
    async createAgent(draft: AgentDraft): Promise<AgentDefinition> {
        const id = draft.id ?? this.agentId(draft.name);
        if (!id) { throw new Error('Cannot derive agent ID from the given name.'); }
        const agent: AgentDefinition = {
            id,
            name: draft.name,
            description: draft.description,
            prompt: draft.prompt,
            defaultLLM: draft.defaultLLM ?? 'google/gemini-2.5-flash',
            showInChat: draft.showInChat ?? true,
        ...
        await fs.promises.writeFile(
            path.join(this.agentsDir, `${id}.json`),
            JSON.stringify(agent, undefined, 2),
            'utf-8'
        );
    ...
    async updateAgent(id: string, patch: Partial<AgentDraft>): Promise<AgentDefinition> {
        ...
        let promptHistory = existing.promptHistory ?? [];
        let promptVersion = existing.promptVersion ?? 1;
    ...
    async deleteAgent(id: string): Promise<void> {
        if (ConfigPlaneServiceImpl.BUILTIN_AGENT_IDS.has(id)) {
    ...
    protected async loadAgent(agentId: string): Promise<AgentDefinition> {
    ...
    async addDirective(agentId: string, text: string, source: 'manual' | 'reflector' = 'manual'): Promise<AgentDirective> {
        ...
        const directives = [...(agent.directives ?? []), directive];
    ...
    async updateDirective(agentId: string, directiveId: string, text: string): Promise<AgentDirective> {
        const agent = await this.loadAgent(agentId);
        const directives = (agent.directives ?? []).map(d =>
            d.id === directiveId ? { ...d, text: text.trim(), evalScore: undefined } : d
        );
    ...
    async removeDirective(agentId: string, directiveId: string): Promise<void> {
        const agent = await this.loadAgent(agentId);
        const directives = (agent.directives ?? []).filter(d => d.id !== directiveId);
        await this.updateAgent(agentId, { directives });
    }
    ...
    async approveDirective(agentId: string, directiveId: string): Promise<AgentDirective> {
        const agent = await this.loadAgent(agentId);
        const directives = (agent.directives ?? []).map(d =>
            d.id === directiveId ? { ...d, status: 'active' as const } : d
        );
        await this.updateAgent(agentId, { directives });
        const found = directives.find(d => d.id === directiveId);
        if (!found) { throw new Error(`Directive '${directiveId}' not found.`); }
        return found;
    }
    ...
    async rejectDirective(agentId: string, directiveId: string): Promise<AgentDirective> {
        const agent = await this.loadAgent(agentId);
        const directives = (agent.directives ?? []).map(d =>
            d.id === directiveId ? { ...d, status: 'rejected' as const } : d
        );
        await this.updateAgent(agentId, { directives });
        const found = directives.find(d => d.id === directiveId);
        if (!found) { throw new Error(`Directive '${directiveId}' not found.`); }
    ...
    async listFeedback(agentId: string): Promise<AgentFeedback[]> {
        ...
        try {
            ...
            return raw.split('\n').filter(Boolean).map(line => JSON.parse(line) as AgentFeedback);
        } catch {
            return [];
        }
    }

    async getAgentTrust(agentId: string): Promise<TrustScore> {
        const feedback = await this.listFeedback(agentId);
        return computeTrustScore(feedback);
    }
     ...
     */
    protected async bootstrapAgents(): Promise<void> {
        await fs.promises.mkdir(this.agentsDir, { recursive: true });
     ...
     * always preserved.
     */
    protected async ensureBuiltinAgent(def: AgentDefinition): Promise<void> {
        ...
        try {
            existing = JSON.parse(await fs.promises.readFile(filePath, 'utf-8')) as AgentDefinition;
        } catch {
        ...
        const userEdited = (existing.promptVersion ?? 1) > 1
            || (existing.promptHistory?.length ?? 0) > 0;
    ...
    protected soulPromptSection(agent: AgentDefinition): string {
        const soul: AgentSoul | undefined = agent.soul;
        if (!soul) { return ''; }
        const lines: string[] = ['', '## Identity'];
        if (soul.role) { lines.push(`**Role**: ${soul.role}`); }
        if (soul.domain) { lines.push(`**Domain**: ${soul.domain}`); }
        if (soul.persona) { lines.push(`**Persona**: ${soul.persona}`); }
        if (soul.coreValues?.length) {
            lines.push('', '**Core Values**');
            for (const v of soul.coreValues) { lines.push(`- ${v}`); }
        }
        return lines.join('\n');
    }
    ...
    protected directivesPromptSection(agent: AgentDefinition): string {
        const { selected, omittedCount } = selectDirectivesForPrompt(
            agent.directives ?? [],
            { soul: agent.soul, description: agent.description },
        );
        ...
        if (omittedCount > 0) {
            lines.push(
                '',
                `_(${omittedCount} additional lower-priority directive${omittedCount === 1 ? '' : 's'} ` +
    ...
    protected skillsPromptSection(agent: AgentDefinition): string {
        ...
        const summaries = this.cachedSkills ?? [];
        const lines: string[] = [
            '',
            '## Your Skills',
    ...
    protected toolsPromptSection(agent: AgentDefinition): string {
        ...
        const defs = this.cachedToolDefs ?? [];
        const lines: string[] = [
            '',
            '## Your Tools',
        ...
        for (const name of assigned) {
            ...
            if (tool) {
                ...
                const paramList = tool.params.length > 0
                    ...
                    : '';
                const backing = (tool.kind ?? 'http') === 'script'
                    ? `[${tool.runtime ?? 'python'} script]`
                    : `[${tool.method ?? 'GET'} via integration ${tool.integrationId}]`;
     ...
     */
    protected async regenerateCustomAgentsYaml(): Promise<void> {
        const agents = await this.listAgents();
    ...
    protected syncTheiaSettings(): void {
        this.doSyncTheiaSettings().catch(e =>
            console.warn('[cc] Theia settings sync failed:', e)
        );
    }

    protected async doSyncTheiaSettings(): Promise<void> {
        ...
        if (resolved) {
            ...
            // OpenAI official
            const openaiKey = values.get('OPENAI_API_KEY') ?? '';
            if (openaiKey) {
            ...
            // Anthropic
            const anthropicKey = values.get('ANTHROPIC_API_KEY') ?? '';
            if (anthropicKey) {
            ...
            const existingCustom = (settings['ai-features.openAiCustom.customOpenAiModels'] as Array<Record<string, unknown>> | undefined) ?? [];
            const userCustom = existingCustom.filter(m =>
                !(managedGeminiIds.has(m['id'] as string)
                    && typeof m['url'] === 'string'
                    && (m['url'] as string).includes('generativelanguage.googleapis.com')));
            ...
            if (geminiKey) {
                const currentAliases = (settings['ai-features.languageModelAliases'] as Record<string, unknown> | undefined) ?? {};
                const managedAliases: Record<string, { selectedModel: string }> = {
        ...
        for (const rec of enabledMcp) {
            ...
            const transport = rec.values['transport'] ?? 'stdio';
            if (transport === 'http') {
                mcpServers[rec.name] = {
                    serverUrl: rec.values['url'] ?? '',
                    ...(secrets['token'] ? { serverAuthToken: secrets['token'] } : {})
                };
            } else {
                // stdio
                const cmd = rec.values['command'] ?? '';
                const argsRaw = (rec.values['args'] ?? '').trim();
                const args = argsRaw ? argsRaw.split(/\s+/) : [];
                const envLines = (rec.values['env'] ?? '').trim();
    ...
    protected static readonly MAX_OUTPUT_BYTES = 100_000;
    /** Maximum bytes returned from a fetched URL body. */
    protected static readonly MAX_FETCH_BYTES = 200_000;
    /** Hard time limit for a single command, in milliseconds. */
    protected static readonly COMMAND_TIMEOUT_MS = 120_000;
    ...
    async executeCommand(command: string, cwd?: string): Promise<CommandResult> {
        ...
        return new Promise<CommandResult>(resolve => {
            exec(command, {
                cwd: workdir,
                timeout: ConfigPlaneServiceImpl.COMMAND_TIMEOUT_MS,
                maxBuffer: ConfigPlaneServiceImpl.MAX_OUTPUT_BYTES,
                windowsHide: true,
                env: process.env
            }, (error, stdout, stderr) => {
                const err = error as (NodeJS.ErrnoException & { code?: number; killed?: boolean; signal?: string }) | null;
                const timedOut = !!err && (err.killed === true || err.signal === 'SIGTERM');
                const exitCode = err && typeof err.code === 'number' ? err.code : (err ? null : 0);
                resolve({
                    stdout: this.truncate(stdout ?? '', ConfigPlaneServiceImpl.MAX_OUTPUT_BYTES),
                    stderr: this.truncate(stderr ?? '', ConfigPlaneServiceImpl.MAX_OUTPUT_BYTES),
                    exitCode,
                    timedOut
                });
            });
    ...
    async fetchUrl(url: string): Promise<FetchResult> {
        ...
        try {
            ...
            const body = this.truncate(raw, ConfigPlaneServiceImpl.MAX_FETCH_BYTES);
            return {
                status: response.status,
                contentType,
                body,
    ...
    async readProjectFile(relPath: string): Promise<string> {
        const abs = this.safeResolve(relPath);
        return fs.promises.readFile(abs, 'utf-8');
    }

    async writeProjectFile(relPath: string, content: string): Promise<void> {
        const abs = this.safeResolve(relPath);
        await fs.promises.mkdir(path.dirname(abs), { recursive: true });
        await fs.promises.writeFile(abs, content, 'utf-8');
    }

    async listProjectFiles(relDir?: string): Promise<FileEntry[]> {
        const abs = this.safeResolve(relDir ?? '.');
        const dirents = await fs.promises.readdir(abs, { withFileTypes: true });
        return dirents
            .filter(d => !d.name.startsWith('.git') && d.name !== 'node_modules')
            .map(d => ({ name: d.name, type: d.isDirectory() ? 'directory' as const : 'file' as const }))
     ...
     */
    protected async bootstrapDirectories(): Promise<void> {
    ...
    async listSessions(): Promise<WorkspaceSession[]> {
        ...
        for (const entry of entries.filter(e => e.isDirectory())) {
            ...
            try {
                ...
                sessions.push(JSON.parse(raw) as WorkspaceSession);
            } catch {
                // Synthesise metadata for dirs without a session.json.
                sessions.push({
                    id: entry.name,
                    name: entry.name,
                    mode: 'named' as SessionMode,
        ...
        return sessions.sort((a, b) =>
            a.mode === 'scratch' ? -1 : b.mode === 'scratch' ? 1 : a.name.localeCompare(b.name)
        );
    }

    async createSession(name: string, ephemeral: boolean): Promise<WorkspaceSession> {
    ...
    async createTool(draft: ToolDraft): Promise<ToolDefinition> {
        ...
        this.regenerateCustomAgentsYaml().catch(() => { /* non-fatal */ });
        return tool;
    }

    async updateTool(id: string, patch: Partial<ToolDraft>): Promise<ToolDefinition> {
     ...
     */
    async executeTool(id: string, args: Record<string, unknown>): Promise<ToolExecuteResult> {
        ...
        const queryParams: Record<string, string> = { ...(tool.staticQueryParams ?? {}) };
        for (const param of tool.params) {
        ...
        const bodyFields: Record<string, unknown> = { ...(tool.staticBody ?? {}) };
        ...
        const body = (hasBodyParam || Object.keys(tool.staticBody ?? {}).length > 0)
     ...
     */
    async executeScriptTool(tool: ToolDefinition, args: Record<string, unknown>): Promise<ToolExecuteResult> {
        const runtime = tool.runtime ?? 'python';
        ...
        const integrations = await this.resolveIntegrationsForScript(tool.integrationRefs ?? []);
        ...
        const command = runtime === 'python' ? `python "${scriptFile}"`
            ...
            : `bash "${scriptFile}"`;
        const timeoutMs = Math.min(Math.max(tool.timeoutMs ?? 120_000, 1_000), 600_000);
        const MAX = ConfigPlaneServiceImpl.MAX_OUTPUT_BYTES;

        return new Promise<ToolExecuteResult>(resolve => {
            exec(command, { cwd: scriptDir, timeout: timeoutMs, maxBuffer: MAX, windowsHide: true, env }, (error, stdout, stderr) => {
                const err = error as (NodeJS.ErrnoException & { code?: number; killed?: boolean; signal?: string }) | null;
                const timedOut = !!err && (err.killed === true || err.signal === 'SIGTERM');
                const exitCode = err && typeof err.code === 'number' ? err.code : (err ? 1 : 0);
                const out = this.truncate(stdout ?? '', MAX);
                const errOut = this.truncate(stderr ?? '', MAX);
                if (timedOut) {
     ...
     */
    protected async writeToolFiles(tool: ToolDefinition, scriptDir: string): Promise<void> {
        ...
        try {
            ...
            if (Array.isArray(parsed)) previous = parsed.filter((x): x is string => typeof x === 'string');
     ...
     */
    protected async resolveIntegrationsForScript(ids: string[]): Promise<Record<string, unknown>> {
        ...
        for (const id of ids) {
            ...
            const entry: Record<string, unknown> = {
                ...
                name: rec.name,
                kind: rec.kind,
                baseUrl: rec.values.baseUrl ?? '',
                authType: rec.values.authType ?? 'none',
            };
            try {
                const secrets = await this.store().getDecryptedSecrets(rec.id);
                const authType = rec.values.authType ?? 'none';
                if (authType.startsWith('oauth2')) {
                    try { entry.bearerToken = await this.ensureAccessToken(rec.id); } catch { /* token unavailable */ }
     ...
     */
    protected async ensureToolRequirements(tool: ToolDefinition, scriptDir: string): Promise<string> {
        ...
        const res = await new Promise<{ code: number | null; stdout: string; stderr: string }>(resolve => {
            exec(cmd, { cwd: scriptDir, timeout: 300_000, maxBuffer: ConfigPlaneServiceImpl.MAX_OUTPUT_BYTES, windowsHide: true, env: process.env },
                (error, stdout, stderr) => {
        ...
        if (res.code !== 0) {
            return this.truncate(res.stderr || res.stdout || `install exited ${res.code}`, 4_000);
        }
