...
- control_plane/src/components/EmptyState.tsx -- THE empty-state box (icon · message · hint · one action), shared so a surface with nothing on it looks the same in every app. **Two states, never one**: a filtered-to-nothing surface says so and offers the control that undoes it; a genuinely empty one does not mention filters. WHICH state a surface is in, and its wording, stays the app's own pure module (`app/projects/lib/emptyState.ts`, unit-tested — vitest here is node-env, so a component test cannot run). Icon names must be mapped in `lib/theme/icon-data/registry.json` for every pack or the glyph silently falls back to Lucide. ⚠️ `/tasks`' `ItemList.tsx` still holds the local `NoMatchState`/`EmptyState` pair this was promoted from, and `projects/page.tsx`'s `renderState()` seam is still inline — both are owed
