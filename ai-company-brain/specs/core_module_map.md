...
|---|---|---|---|---|---|
| A1 | **Agent loop / executor** — run_agent_stream/run_agent, 3-tier dispatch (native MAF stream / Copilot SDK / batch shim), watchdogs, retries | `apps/orchestrator/orchestrator/executor.py` (~4.2k lines) | Agent Loop | **B** | Monolith; watchdog policy still per-tier; P1-6 Copilot session continuity; no loop detection. Duplicated stream paths eliminated (Phase 2). **P1-2 closed 2026-07-02**: cross-worker control bus (Redis pub/sub) — cancel + HITL answer now reach the worker that owns the run instead of silently no-op'ing under >1 worker |
| A2 | **Event translation** — `orchestrator/event_translator.py` (ONE canonical mapping + TranslatorHooks), per-message dedup | `event_translator.py`; folds now segment-native + id-less fallback in `chat/route.ts` + `chatStream.ts` + `chat_fold.py` | Agent Loop / AG-UI | **A−** | Unified 2026-07-02 (Phase 2, parity evals). Phase 3b (2026-07-02) made rendering segment-native: the client prefers real message segments; the fold is demoted to the documented id-less fallback (litellm/langgraph). Remaining: unified per-tier watchdog policy (shared w/ A1) |
