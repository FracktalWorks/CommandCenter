...
- **Missing (historical — resolved):** `get_current_user` never rejects (`acb_auth/deps.py:76`); it only labels. So mutation‑approve (`agent.py:1852`, `git push`), the memory API (`memory.py`, IDOR), and `/agent/webhook/{source}` (`agent.py:2522`) were anonymous‑reachable. `/v1` had no auth at all. **Update 2026-08-01 (doc-truth pass):** this paragraph is resolved history — every systemic item in the Residual list below is ✅; the honest remaining residual is that in‑process agents can read the identity token until BO‑7.
...
- **Cross‑ref:** "BO‑4" in `FOUNDATION_AUDIT_REPORT.md` §5 refers to this item.
