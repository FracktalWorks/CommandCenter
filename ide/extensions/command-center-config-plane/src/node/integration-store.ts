...
export class IntegrationStore {
    ...
    async create(draft: IntegrationDraft): Promise<IntegrationRecord> {
    ...
    async update(id: string, patch: Partial<IntegrationDraft>): Promise<IntegrationRecord> {
        const file = await this.read();
    ...
    async setEnabled(id: string, enabled: boolean): Promise<IntegrationRecord> {
    ...
    protected validateKind(kind: IntegrationKind): void {
        if (!INTEGRATION_KIND_SPECS.some(s => s.kind === kind)) {
            throw new Error(`Unknown integration kind: ${kind}`);
        }
    }

    /** Secret vs non-secret declared field keys for a kind. */
    protected fieldKeys(kind: IntegrationKind): { secret: Set<string>; nonSecret: Set<string> } {
        const spec = INTEGRATION_KIND_SPECS.find(s => s.kind === kind);
        const secret = new Set((spec?.fields ?? []).filter(f => f.type === 'secret').map(f => f.key));
        const nonSecret = new Set((spec?.fields ?? []).filter(f => f.type !== 'secret').map(f => f.key));
     ...
     */
    protected routeBuckets(
        kind: IntegrationKind,
        values?: Record<string, string>,
        secrets?: Record<string, string>
    ): { values: Record<string, string>; secretsPlain: Record<string, string> } {
        const { secret, nonSecret } = this.fieldKeys(kind);
        const outValues: Record<string, string> = {};
        const outSecrets: Record<string, string> = {};
        const merged: Record<string, string> = { ...(values ?? {}), ...(secrets ?? {}) };
    ...
    protected async read(): Promise<StoreFile> {
        try {
            ...
            for (const integration of integrations) {
                this.migrateBuckets(integration as StoredIntegration);
            }
            return {
    ...
    async getDecryptedSecrets(id: string): Promise<Record<string, string>> {
        ...
        for (const [key, enc] of Object.entries(stored.secrets)) {
            try { out[key] = this.decrypt(enc); } catch { /* skip corrupt */ }
        }
        return out;
    }
}
