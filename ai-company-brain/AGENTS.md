...
| [`workflows_app.md`](specs/workflows_app.md) | **Workflows app (`/workflows`)** — visual automation builder: DB-persisted graphs compiled to MAF Workflows, webhook/schedule/manual triggers, Module Studio (conversational pure-transform code modules), served node catalog. Engineering RFC: `docs/workflow-editor/README.md`. Policy: ADR-028 | 🔄 v1 slice built (migration 132, gateway `routes/workflows/`, `/workflows` editor); event triggers + approval node next |
